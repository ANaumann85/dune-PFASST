...
typedef Dune::BlockVector<Dune::FieldVector<double,1> > VectorType;
 ...
 class fe_manager{
    ...
    typedef TransferOperatorAssembler<GridType> TransferOperator;
	  ...
	  std::shared_ptr<TransferOperator> transfer;
    ...
    fe_manager(std::shared_ptr<GridType> g, const size_t nelements, size_t nlevels=1, size_t base_order=1)
	  :grid(g), fe_basis(nlevels), n_levels(nlevels)
	  {
	    //Konstruktor
	  ...
	  void create_transfer(){
	    transfer = std::make_shared<TransferOperator>(*grid);
